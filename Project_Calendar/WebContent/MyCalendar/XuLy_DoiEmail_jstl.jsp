...
<% request.setCharacterEncoding("UTF-8"); %>
	<sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
		...
		<c:if test="${result_0.rowCount != 1}">
            ...
            <c:redirect url="DoiEmail.jsp" >
                <c:param name="errMsg" value = "Nhập email cũ không chính xác! Vui lòng thử lại!" />
            </c:redirect>
        </c:if> 
		
<sql:update dataSource="${snapshot}" var="result1">
        	
        	Update NguoiDung set Email = ?
        ...
        <c:if test="${result1>=1}">
            ...
            <c:redirect url="DoiEmail.jsp" >
                <c:param name="susMsg" value="Đã đổi email thành công!" />
               <%--  <c:param name="msg_email1" value="${param.email1}" /> --%>
