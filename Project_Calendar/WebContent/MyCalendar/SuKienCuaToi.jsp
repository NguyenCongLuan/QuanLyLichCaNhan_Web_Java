...
DATE_FORMAT(NgayKetThuc,'%m/%d/%Y') AS NgayKetThuc, GioKetThuc, MoTa from SuKien
...
select s.MaSuKien, s.TenSuKien, s.DiaDiem, DATE_FORMAT(s.NgayBatDau,'%m/%d/%Y') AS NgayBatDau, s.GioBatDau, 
DATE_FORMAT(s.NgayKetThuc,'%m/%d/%Y') AS NgayKetThuc, s.GioKetThuc, s.MoTa, 
s.TenDangNhap from 
SuKien s, MoiKhach_SuKien m where s.MaSuKien = m.MaSuKien and
m.TenDangNhap_KhachMoi = '${sessionScope['loginUser']}';
</sql:query>
				
				<div id = "form2" class="panel panel-default">
					<div class="panel-heading">
						<h3 class="panel-title">Sự kiện được mời</h3>
					</div>
					<div class="panel-body">
						<form id="signupForm1" method="post" class="form-horizontal" action="">
...
<tr id = "dong">
	<td><c:out value="${row1.MaSuKien}"/></td>
	<td><c:out value="${row1.TenSuKien}"/></td>
	<td><c:out value="${row1.DiaDiem}"/></td>
	<td><c:out value="${row1.NgayBatDau}"/></td>
	<td><c:out value="${row1.GioBatDau}"/></td>
	<td><c:out value="${row1.NgayKetThuc}"/></td>
	<td><c:out value="${row1.GioKetThuc}"/></td>
	<td><c:out value="${row1.MoTa}"/></td>
	<td><c:out value="${row1.TenDangNhap}"/></td>
	<td><a href = "XuLy_SuKien_jstl.jsp?id=<c:out value="${row.MaSuKien}"/>">Chấp nhận</a></td>
	<td><a onclick = "Xoa(${row.MaSuKien}); return false;" href = "#" >Từ chối</a></td>
