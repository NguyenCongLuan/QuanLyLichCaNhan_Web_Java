...
<c:forEach var = "row" items = "${result.rows}">
	<sql:update dataSource="${snapshot}" var="result2">
        ...
        <c:if test="${result2>=1}">
            ...
            <c:redirect url="XuLy_SuKien_jstl.jsp" >
               <c:param name="susMsg_1" value="Đã thêm 1 khách vào sự kiện thành công!" />
	          ...
	          <c:otherwise>
	            <c:redirect url="XuLy_SuKien_jstl.jsp" >
	              <c:param name="errMsg_Khach1" value="User bạn mời không tồn tại! Vui lòng thử lại" />
	            </c:redirect>
	          </c:otherwise>
	        </c:choose>
