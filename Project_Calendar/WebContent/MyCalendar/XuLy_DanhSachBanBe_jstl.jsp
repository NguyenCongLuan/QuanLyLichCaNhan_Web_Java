...
user="root" password="trongtri96"/>
			<c:if test="${not empty param.Msg_BanBe}">
			 <% request.setCharacterEncoding("UTF-8"); %>
			<sql:query dataSource ="${snapshot}" var="result">
select HoTen, DiaChi, Email, SoDienThoai from NguoiDung
	        where TenDangNhap='${param.Msg_BanBe}' or HoTen='${param.Msg_BanBe}' 
	        or Email='${param.Msg_BanBe}'
	        or SoDienThoai='${param.Msg_BanBe}';
</sql:query>
<%-- <c:out value="${param.Msg_BanBe}" />
...
<tr id = "dong">
	...
	<td><a href = "TimBanBe.jsp?id_email=<c:out value="${row.Email}"/>">Xem chi tiết</a></td>
	<%-- <td><a onclick = "Xoa(${row.Email}); return false;" href = "#" >Mời tham gia sự kiện</a></td> --%>
